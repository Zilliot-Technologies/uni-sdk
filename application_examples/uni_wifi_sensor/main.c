...
char *sensorVarName = "{Temp,Humid,AL,R,G,B,X,Y,Z,dummy}";
...
int main()
{
  ...
  while(1)
  {
   ...
   if (sendCredFlag){
    ...
    snprintf(credData,sizeof(credData),"{%s,%s,%s}\n",wifi_ssid,wifi_pswd,ubidots_token);
